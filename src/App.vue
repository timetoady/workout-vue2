    ...
    <div id="currentSession">
      <div id="sessionDisplay" v-if="adding === true && currentSess.id">
        ...
        <div class="exerciseList">
          <template
            v-for="(oneExer, index) in currentSess.exersizes.filter(
              (item) => item.exType === 'aerobic'
            )"
          >
            <div class="exerSet" :key="`div-${index + giveRandom(9999999)}`">
              <h5 :key="`exersizeAe-${index + giveRandom(9999999)}`">
                Exercise: {{ oneExer.exercise }}
              </h5>
              <h5 :key="`distance-${index + giveRandom(9999999)}`">
                Distance: {{ oneExer.distance }} {{ oneExer.distanceType }}
              </h5>
              <h5 :key="`duration-${index + giveRandom(9999999)}`">
                Duration: {{ oneExer.duration }}
              </h5>
            </div>
          </template>
          ...
          >
            <div class="exerSet" :key="`div-${index + giveRandom(9999999)}`">
              <h5 :key="`exersizeAr-${index + giveRandom(9999999)}`">
                Exercise: {{ oneExer.exercise }}
              </h5>
              <h5 :key="`set-${index + giveRandom(9999999)}`">
                Sets: {{ oneExer.sets }}
              </h5>
              <h5 :key="`reps-${index + giveRandom(9999999)}`">
                Reps: {{ oneExer.reps }}
              </h5>
              <h5 :key="`weight-${index + giveRandom(9999999)}`">
                Weight: {{ oneExer.weight }}
                {{ oneExer.weightType.toUpperCase() }}
              </h5>
            </div>
          </template>
        </div>
...
export default {
  ...
  methods: {
    addItem(item) {
      ...
      if (
        Object.keys(this.currentSess).length === 0 &&
        this.currentSess.constructor === Object
      ) {
